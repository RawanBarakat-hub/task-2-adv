import { faqQuestions } from "../../data"
import FaqContainer from "../FaqContainer/FaqContainer"
import Question from "../Question/Question"
import TitleSection from "../TitleSection/TitleSection"

const FaqSection = () => {

    const leftColumn = faqQuestions.filter((_, index) => index % 2 === 0)
    const rightColumn = faqQuestions.filter((_, index) => index % 2 !== 0)

    return (
        <section id="faq" className="section_margin">
            <TitleSection 
                info="Solutions For The Doubts"
                title="Frequently Asked Questions"
                description="Find all the essential information you need in our FAQ section, designed to address the most frequently asked questions and help you make informed decisions for your child's education." 
            />

            <FaqContainer>
                <div className="faq_column">
                    {leftColumn.map((faqQuestion, index) => (
                        <Question
                            key={index}
                            question={faqQuestion?.question}
                            answer={faqQuestion?.answer}
                        />
                    ))}
                </div>

                <div className="faq_column">
                    {rightColumn.map((faqQuestion, index) => (
                        <Question
                            key={index}
                            question={faqQuestion?.question}
                            answer={faqQuestion?.answer}
                        />
                    ))}
                </div>
            </FaqContainer>

        </section>
    )
}

export default FaqSection